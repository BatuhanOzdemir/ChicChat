...
| Step 7 | **Console authentication** (deferred from Step 5) — a shared passcode over every operator surface, failing closed in production. Three defects found while deploying: one merchant's expired WhatsApp token aborted the *whole* inactivity sweep, now isolated per session with a `failed` count; the Supabase pooler accepted a **plaintext** connection when the URI carried no SSL parameters; and every operator surface was a navigational dead end (no link home, none sideways), now one shared nav bar over an `(app)` route group. **R6b a third time**, and only a hosted database could have exposed it: `begin`/`rollback` makes the tests independent of each other but not of what is already committed, so a leftover session on a fixed fake phone number silently resumed instead of starting fresh, every scripted answer landed one step early, and the assertion failed while the application was behaving correctly |

| Step 8 | **R13 (partly)** — the KVKK disclosure now actually reaches customers on the first message of every new conversation; `kvkk_url` had been storable and editable since Step 3 but never left the database. Retention enforcement and per-phone deletion remain. Found by the first live conversation: the `return` taxonomy's **`reason` field is declared `type: enum` with `enum_values: null`**, so it degrades to a free-text question and captures unconstrained text in a field the schema calls an enum — routing rules matching on reason values cannot fire reliably (see the Step 8 report). **Fixed on sign-off**: both `reason` fields (`return`, `exchange`) now carry values and render as tappable lists, and the answer is canonicalized like any other enum. `exchange.desired_variant` keeps `enum_values: null` deliberately — it is catalog-driven and gets its values from the connector, so it stays a free-text question until then |
...
**Steps 9 and 10 are deferred by the user's decision on 2026-08-28.** Step 9
(İkas connector) was skipped for want of a store to connect to. Step 10 (Flow
item picker) inherits that dependency — its gate needs a customer picking real
line items out of a real dev-store order — and is additionally blocked while
Meta returns `API access blocked` at the app level. Neither is abandoned; both
resume when a dev store and Meta access exist. Everything Steps 9 and 10 would
have delivered is listed under R22 and remains missing from v0.2.
...
per-merchant) — still deferred, and only necessary when merchants bring their
own Meta apps.

Console **authentication** landed in Step 7 as a single shared passcode, which
keeps case data off the open internet but does **not** separate operators: the
merchant switcher is still a cookie, so anyone past the passcode can act as any
tenant. That is why the switcher validates its input against the real merchant
list rather than trusting it. Per-user identity and per-merchant permissions are
v0.3.
